...
function WarehouseModal({ warehouse, onClose, onSave }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (isEdit) {
        const numericId = parseInt(warehouse.id.replace('W', ''));
    ...
    >
      ...
      >
        ...
        <div className="space-y-4">
          <div>
            <label className="text-sm font-medium text-slate-300 mb-1.5 block">Nume depozit</label>
            <input
              className={`w-full bg-slate-700/60 border ${errors.name ? 'border-red-500' : 'border-slate-600'} rounded-xl px-4 py-3 text-white placeholder-slate-500 focus:outline-none focus:border-emerald-500 transition-colors`}
              placeholder="ex: Depozit Central Cluj"
              value={form.name}
          ...
          <div>
            <label className="text-sm font-medium text-slate-300 mb-1.5 block">Locație</label>
            <input
              className={`w-full bg-slate-700/60 border ${errors.location ? 'border-red-500' : 'border-slate-600'} rounded-xl px-4 py-3 text-white placeholder-slate-500 focus:outline-none focus:border-emerald-500 transition-colors`}
              placeholder="ex: Cluj-Napoca, Str. Industriilor 12"
              value={form.location}
        ...
        <div className="flex gap-3 mt-8">
          ...
          <button
            ...
            disabled={loading}
            className="flex-1 px-4 py-3 rounded-xl bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 text-white font-semibold transition-colors flex items-center justify-center gap-2 cursor-pointer"
          >
  ...
  return (
    ...
    >
      ...
      >
        ...
        <h2 className="text-xl font-bold text-white text-center mb-2">Șterge depozit?</h2>
        <p className="text-slate-400 text-center text-sm mb-6">
          <span className="text-white font-medium">{warehouse.name}</span> va fi șters permanent. Toate produsele asociate vor fi afectate.
        </p>
        <div className="flex gap-3">
...
export default function WarehousesPage({ onNavigate }) {
  ...
  const fetchWarehouses = async () => {
    ...
    try {
      ...
      toast.error('Nu s-au putut încărca depozitele');
    } finally {
      setLoading(false);
    }
  };
...
useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <nav className="sticky top-0 z-40 backdrop-blur-md bg-white/70 border-b border-slate-200/60">
        <div className="max-w-7xl mx-auto px-8 h-18 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-8">
            <button onClick={() => onNavigate('warehouses')} className="transition-colors cursor-pointer font-semibold text-sm text-indigo-600">Depozite</button>
            <button onClick={() => onNavigate('suppliers')} className="transition-colors cursor-pointer font-semibold text-sm text-slate-600 hover:text-indigo-600">Furnizori</button>
        ...
        >
          <div>
            ...
            <p className="text-slate-500 mt-1 text-sm">
              {warehouses.length} depozit{warehouses.length !== 1 ? 'e' : ''} înregistrat{warehouses.length !== 1 ? 'e' : ''}
            </p>
          </div>
          <button
        ...
        {loading ? (
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-32">
            ...
            <p className="text-slate-500 font-medium">
              {search ? 'Niciun rezultat găsit' : 'Niciun depozit înregistrat'}
            </p>
            {!search && (
            ...
            <AnimatePresence>
              {filtered.map((warehouse, i) => (
                ...
                >
                  ...
                  <h3 className="font-bold text-slate-900 text-lg leading-tight mb-1">{warehouse.name}</h3>
                  <div className="flex items-center gap-1.5 text-slate-500 text-sm">
                    <Icon name="MapPin" className="w-3.5 h-3.5 shrink-0" />
                    <span className="truncate">{warehouse.location}</span>
                  </div>

                  {/* Buton navigare la produsele depozitului */}
                  <button
                    onClick={() => onNavigate('products', { warehouseId: warehouse.id })}
                    className="w-full mt-4 flex items-center justify-center gap-1.5 px-3 py-2.5 rounded-xl text-sm font-medium bg-emerald-50 text-emerald-700 hover:bg-emerald-100 transition-colors cursor-pointer"
                  >
                  ...
                  <div className="flex gap-2 mt-3 pt-3 border-t border-slate-100">
                    <button
                      onClick={() => setModal({ type: 'edit', warehouse })}
      ...
      <AnimatePresence>
        {modal?.type === 'create' && (
          <WarehouseModal onClose={() => setModal(null)} onSave={fetchWarehouses} />
        )}
        {modal?.type === 'edit' && (
          <WarehouseModal warehouse={modal.warehouse} onClose={() => setModal(null)} onSave={fetchWarehouses} />
        )}
        {modal?.type === 'delete' && (
          <DeleteConfirm warehouse={modal.warehouse} onClose={() => setModal(null)} onDeleted={fetchWarehouses} />
        )}
      </AnimatePresence>
    </div>
  );
